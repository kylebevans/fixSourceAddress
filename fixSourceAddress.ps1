...
$serviceDisplayName = "FixSourceAddress"
$ServiceDescription = "Ensures only the Loopback0 interface is used to source traffic"
...
if ($Service) {                 # Run the service
  ...
  try {
    ...
    # Set the Ethernet interfaces to not be the source address
    # for packets since we want the Loopback adapter IP address
    # to be the source for all traffic. Need to renew IP after.
    # Also restart the RemoteAccess service if there is no
    # default route.

    # A timer runs and sends a "TimerTick" Event every period.
    # The service waits for an event, and when it gets "TimerTick"
